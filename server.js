...
// ---------------------------------------------------------------------------
// SECTION 0 — Context (no login gate)
//
// The password-based login was the source of a persistent, unresolved
// "empty response" failure on the deployed environment — removed for now
// rather than leaving the app unusable while that gets sorted separately.
// There is no server-verified identity anymore: the client sends which
// region it's working as (and whether it's "Admin") as plain headers, and
// the server trusts them at face value. The status-based workflow locks
// (below) still work exactly as before — this only removes the password
// checkpoint in front of them, not the workflow itself. Reinstating real
// auth later just means putting a verified check back in front of
// `readContext` — everything downstream is unchanged.
// ---------------------------------------------------------------------------
function readContext(req, res, next) {
  req.role = req.headers['x-role'] === 'admin' ? 'admin' : 'region';
  req.region = req.headers['x-region'] || null;
  next();
}

function requireAdmin(req, res, next) {
  if (req.role !== 'admin') return res.status(403).json({ error: 'Admin context required for this action.' });
  next();
}
...
api.use(readContext); // no login gate for now — see SECTION 0 above
